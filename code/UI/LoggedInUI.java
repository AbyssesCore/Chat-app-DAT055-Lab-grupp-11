...
class LoggedInUI implements IChatUI {
	...
	public void renderNewText(String username, String content, int hour, int minute, boolean isMe) {
		...
		JComponent contentComp = new JLabel(content);
		contentComp.setOpaque(false);
		
		JPanel nameAndTime = new JPanel(new BorderLayout());
		
		nameAndTime.setBackground(isMe ? new Color(220, 238, 255) : Color.WHITE);
		
		nameAndTime.add(name, BorderLayout.WEST);
		
		JLabel time = new JLabel("      " + hour + ":" + String.format("%02d", minute));
        time.setFont(time.getFont().deriveFont(Font.BOLD, 12f));
        time.setForeground(new Color(70, 75, 85));
		
		nameAndTime.add(time, BorderLayout.EAST);
        
		
        bubble.add(nameAndTime);
        bubble.add(Box.createVerticalStrut(4));
        bubble.add(contentComp);
	...
	public void renderImg(String username, BufferedImage imgContent, int hour, int minute, boolean isMe)  {
		...
		Dimension d = new Dimension(imgContent.getWidth(), imgContent.getHeight());
		...
		JComponent content = new JLabel( new ImageIcon(new ImageIcon(imgContent).getImage().getScaledInstance(widthLim, heightLim, java.awt.Image.SCALE_SMOOTH)) );
